...
def test_get_moneyhub_client_is_lazy_and_reused(monkeypatch):
...
def test_startup_validation_rejects_partial_configuration(monkeypatch):
    monkeypatch.setenv("MONEYHUB_CLIENT_ID", "client-id")
    monkeypatch.delenv("MONEYHUB_CLIENT_SECRET", raising=False)

    with pytest.raises(RuntimeError, match="MONEYHUB_CLIENT_ID and MONEYHUB_CLIENT_SECRET must be set"):
        transactions_route.validate_moneyhub_configuration()


def test_startup_validation_allows_unconfigured_optional_integration(monkeypatch):
