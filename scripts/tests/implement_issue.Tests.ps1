# Unit tests for the file discovery logic in ../developer_tools/e_implement_issue_using_local_llm.ps1
# (direct path matching + basename fallback via `git ls-files`, added by PR #4293).
#
# e_implement_issue_using_local_llm.ps1 is a top-level script with real side effects (git checkout,
...
#
# Constraint: this file must never modify e_implement_issue_using_local_llm.ps1 (tests only).

BeforeAll {
    $script:ScriptUnderTest = (Resolve-Path (Join-Path $PSScriptRoot '..' 'developer_tools' 'e_implement_issue_using_local_llm.ps1')).Path
    ...
    # If e_implement_issue_using_local_llm.ps1 is restructured, these IndexOf calls stop finding the
    ...
    if ($startIndex -lt 0) {
        throw "Could not locate start marker '$startToken' in e_implement_issue_using_local_llm.ps1 - the file discovery logic may have moved. Update the extraction markers in this test file."
    }

    $endAnchor = $raw.IndexOf($endToken)
    if ($endAnchor -lt 0) {
        throw "Could not locate end marker '$endToken' in e_implement_issue_using_local_llm.ps1 - the file discovery logic may have moved. Update the extraction markers in this test file."
    }
...
Describe 'e_implement_issue_using_local_llm.ps1 file discovery logic' {
