...
def mock_google_verify(monkeypatch, request):
    ...
    Those tests live in ``tests/test_google_auth.py`` and ``tests/backend/test_auth*.py``
    and expect the application's :func:`backend.auth.verify_google_token` helper to run
    ...
    patching for tests defined in those modules.
    """

    # ``request`` points at the currently executing test.  When the test file
    # is ``test_google_auth.py`` or ``test_auth*.py`` we leave the real ``verify_google_token`` in
    ...
    if fspath and (fspath.basename == "test_google_auth.py" or fspath.basename.startswith("test_auth")):
