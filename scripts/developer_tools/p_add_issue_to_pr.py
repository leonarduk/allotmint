...
def run_gh(args: list[str]) -> subprocess.CompletedProcess[str]:
    ...
    try:
        ...
        return subprocess.CompletedProcess(cmd, returncode=124, stdout="", stderr=f"gh {' '.join(args)} timed out")
...
def build_issue_body(pr: PullRequest) -> str:
    ...
    parts = [
        "## What",
        "",
        f"This issue was retroactively created to link PR #{pr.number}, which had " "no linked issue.",
        "",
        what,
        "",
        "## Why",
...
def existing_labels(owner: str, repo: str, labels: list[str]) -> list[str]:
    """Return configured labels that exist, warning about labels that do not."""
    if not labels:
        return []
    ...
    if result.returncode != 0:
        print(
            "WARNING: could not verify configured labels; creating the issue "
            f"without labels: {result.stderr.strip()}",
            file=sys.stderr,
        )
        return []

    try:
        available = {item["name"].casefold() for item in json.loads(result.stdout)}
    except (json.JSONDecodeError, KeyError, TypeError) as exc:
        print(
            "WARNING: could not parse the repository label list; creating the issue " f"without labels: {exc}",
            file=sys.stderr,
        )
        return []
    ...
    for label in labels:
        if label.casefold() in available:
            ...
            print(
                f"WARNING: label {label!r} does not exist in {owner}/{repo}; skipping it",
                file=sys.stderr,
            )
    return valid
...
def create_issue(owner: str, repo: str, title: str, body: str, labels: list[str]) -> int | None:
    ...
    try:
        ...
        for label in existing_labels(owner, repo, labels):
            args += ["--label", label]
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="Create and link issues for open PRs that don't reference one")
    parser.add_argument(
        "--yes",
        action="store_true",
        help="Actually create issues and edit PR descriptions. Without this flag, " "runs in dry-run mode.",
    )
    parser.add_argument(
        "--repo",
        default=None,
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Label to apply to created issues (repeatable). Defaults to 'bug' and " f"'{DEFAULT_VALUE_LABEL}'.",
    )
