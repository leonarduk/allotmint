...
def test_audit_report_pdf_contains_all_section_titles(monkeypatch):
    ...
    monkeypatch.setattr(
        ...
        lambda owner, confidence, include_cash=True: {"confidence": confidence, "1d": 800.0, "10d": 2530.0},
    )
...
def test_audit_report_json_section_order_and_presence(monkeypatch):
    ...
    monkeypatch.setattr(
        reports.risk_mod,
        "compute_portfolio_var",
        lambda owner, confidence, include_cash=True: {"confidence": confidence, "1d": 200.0, "10d": 630.0},
    )
