...
def _make_client(tmp_path, monkeypatch):
    ...
    monkeypatch.setenv("MONEYHUB_TOKENS_STORAGE_URI", f"file://{template}")
    monkeypatch.setenv("MONEYHUB_CLIENT_ID", "test-client")
    monkeypatch.setenv("MONEYHUB_CLIENT_SECRET", "test-secret")
    monkeypatch.setattr(transactions_route, "_moneyhub_client_instance", None)
    app = create_app()
    return TestClient(app)


def test_moneyhub_client_is_lazily_created_once(monkeypatch):
    created = []

    class FakeMoneyhubClient:
        def __init__(self, **kwargs):
            created.append(kwargs)

    monkeypatch.setenv("MONEYHUB_CLIENT_ID", "client-id")
    monkeypatch.setenv("MONEYHUB_CLIENT_SECRET", "client-secret")
    monkeypatch.setattr(transactions_route, "MoneyhubClient", FakeMoneyhubClient)
    monkeypatch.setattr(transactions_route, "_moneyhub_client_instance", None)
...
def test_moneyhub_client_requires_credentials(monkeypatch):
