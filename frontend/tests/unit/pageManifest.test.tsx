...
describe('pageManifest', () => {
  it('keeps route metadata unique and covers every mode', () => {
    ...
    expect(PAGE_MANIFEST.map((entry) => entry.mode).sort()).toEqual(
      [...MODES].sort()
    );
  });
  ...
  it('keeps menu metadata aligned with configured menu sections', () => {
    for (const entry of PAGE_MANIFEST) {
      ...
      expect(MENU_CATEGORY_ORDER[entry.menu.section]).toContain(
        entry.menu.category
      );
  ...
  it('derives modes and paths from the same manifest', () => {
    ...
    expect(deriveModeFromPathname('/input')).toBe('transactions');
    ...
    expect(buildPathForMode('owner', { owner: 'alex' })).toBe(
      '/portfolio/alex'
    );
