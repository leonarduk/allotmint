...
| --- | --- | --- |
| `logs:FilterLogEvents` | Log group ARNs for all three Lambdas | Post-deploy log-dump CI step (`aws logs filter-log-events`) |
| `logs:DescribeLogStreams` | Same log group ARNs | Listing streams when diagnosing missing log output |

The log group ARNs are not fixed strings — they are auto-generated by CDK.
Query them from the stack outputs after the first deploy:
...
Minimum inline policy JSON to attach to the deploy role:
...
Replace `<region>`, `<account>`, and the log group name placeholders with the
values from the stack outputs above.  Without `logs:FilterLogEvents` the
CI step continues silently (it uses `continue-on-error`) but produces no
diagnostic output.
