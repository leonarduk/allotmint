...
class Config:
    ...
    # Regex alternative to the explicit ``cors_origins`` allowlist, passed
    # straight to CORSMiddleware's ``allow_origin_regex``. Lets a dev machine
    # running several frontends cover a whole port range without enumerating
    # every port (vite walks 5173, 5174, ... when ports are already taken).
...
def _load_cors_origin_regex(data: Dict[str, Any]) -> Optional[str]:
    ...
    Mirrors :func:`_load_cors_origins`: a ``cors.origin_regex`` key in
    ``config.yaml``, overridable by ``CORS_ORIGIN_REGEX``. Compiled here so a
    malformed pattern fails at config load with a clear message rather than on
    ...
    if origin_regex is not None:
        try:
            ...
            raise ValueError(f"Invalid CORS origin regex {origin_regex!r}: {exc}") from exc
