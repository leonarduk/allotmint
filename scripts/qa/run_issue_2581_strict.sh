...
capture_broker_snapshot() {
  ...
  cp "$BROKER_SNAPSHOT_PATH" "$RUN_DIR/broker_snapshot.txt"
}
...
check_pdf_endpoint_reconciliation() {
  python3 - \
...
audit_path, portfolio_path, sectors_path, regions_path, var_path = sys.argv[1:]
...
main() {
  ...
  capture_broker_snapshot
  if [[ -f "$RUN_DIR/broker_snapshot.txt" ]] && ! check_portfolio_vs_broker_snapshot 2>>"$RUN_DIR/portfolio_vs_broker.check.log"; then
  ...
  if [[ -f "$RUN_DIR/audit_report.json" ]] && ! check_pdf_endpoint_reconciliation 2>>"$RUN_DIR/audit_reconciliation.check.log"; then
