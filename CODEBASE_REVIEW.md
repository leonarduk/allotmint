...
- **Priority:** high
- **Stage:** v1 blocker
- **Local vs Lambda entrypoint (investigated #4409):** `backend/app.py` is **the same app used in production**. The Lambda entrypoint is `backend/lambda_api/handler.py` (`CMD ["backend.lambda_api.handler.lambda_handler"]` in `backend/Dockerfile.lambda`), which calls `backend.app.create_app()` and wraps the result with [Mangum](https://github.com/jordaneremieff/mangum) on the first invocation. There is no separate lighter handler — local dev (`uvicorn`) and AWS Lambda both run the same FastAPI application object. Initialization is deferred to the first Lambda invocation (not the INIT phase) specifically because Lambda's INIT phase has a hard 10-second limit that the full startup sequence — config loading, snapshot warming, router registration — can exceed on a cold start. This means that all coupling described below applies to the production code path, not only to local development.
...
- **Impact:** Startup regressions are likely to be discovered late because the behavior is coupled together and runs on every Lambda cold start in production. Failures in one concern can mask another, and onboarding contributors must understand too much global state before changing a single part of app creation.
