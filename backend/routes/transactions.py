...
def _get_moneyhub_client() -> MoneyhubClient:
    ...
    if _moneyhub_client_instance is None:
        ...
        if not client_id or not client_secret:
            raise RuntimeError(
                "MONEYHUB_CLIENT_ID and MONEYHUB_CLIENT_SECRET must be set"
            )
        _moneyhub_client_instance = MoneyhubClient(
            client_id=client_id,
            client_secret=client_secret,
            base_url=os.environ.get(
                "MONEYHUB_BASE_URL", "https://api.moneyhub.co.uk"
            ),
        )
...
def validate_moneyhub_configuration() -> None:
    """Validate credentials at startup when Moneyhub is enabled.

    With neither credential present, Moneyhub is considered disabled. This
    preserves local and test startup while still making the first attempted
    Moneyhub use fail clearly. If either setting is present, validate both and
    eagerly initialise the otherwise-lazy client during application startup.
    """
    if os.environ.get("MONEYHUB_CLIENT_ID") or os.environ.get(
