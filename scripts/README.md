...
URLs (`http://localhost:6468` and `http://localhost:2568`).
...
SMOKE_TEST_URLS=http://localhost:6468/health,http://localhost:2568 ./scripts/smoke-test.ps1

# Skip the autogenerated suites when only the HTTP 200 probes are required
SMOKE_TEST_URLS=http://localhost:6468/health ./scripts/smoke-test.ps1 -UrlsOnly
...
5. Run:
   ```bash
   python scripts/site_snapshot.py --base-url http://localhost:2568 \
...
# Explicit URL
./scripts/generate-sitemap.ps1 -BaseUrl http://localhost:2568
```
...
2. Make git use that access when it clones `cicaid-pro`. Either:
   ...
   - Use a PAT directly, scoped to just this repo so it can't leak into other
     ...
     `.github/scripts/pip_install_cicaid_pro.sh`; unlike CI, a local `--global`
...
If you don't have (and can't get) access to `cicaid-pro`, `pip install -r
