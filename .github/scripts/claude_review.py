...
All shared logic (prompt building, fetch/retry, verdict finalization) is imported
from the installed ``cidaid_devtools`` package — no local copy is maintained here.
"""
...
def get_anthropic_model() -> str:
    ...
    Anthropic's current models page lists ``claude-sonnet-4-6`` as the latest
    Sonnet API alias, so we default to that stable alias and allow
    ``ANTHROPIC_MODEL`` to override it if the workflow needs to pin or roll
    forward without another code change.
    """
...
def fetch_claude_review(api_key: str, prompt: str) -> str:
    ...
    The workflow is expected to provide ``ANTHROPIC_API_KEY``; HTTP errors are
    ...
    if extra.get("stop_reason") == "max_tokens":
        review = (
            ...
            "Consider increasing `ANTHROPIC_MAX_TOKENS` if this keeps "
            "happening._"
        ).strip()
    return review
...
def main() -> int:
    ...
    if not context.api_key:
        return emit_missing_key_notice("Claude", "ANTHROPIC_API_KEY")
    ...
    return finalize_review(review, "Claude")
