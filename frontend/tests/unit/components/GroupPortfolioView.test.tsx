...
describe("GroupPortfolioView", () => {
  ...
  it("shows a consolidated notice when held instruments have no price history", async () => {
    const mockPortfolio = {
      name: "At a glance",
      accounts: [
        {
    ...
    fetchMock.mockImplementation((input: RequestInfo | URL) => {
      ...
      if (url.includes("/instrument/?ticker=")) {
        const hasHistory = url.includes("ticker=HIST.L");
        return Promise.resolve({
          ok: true,
          json: async () => ({
            prices: hasHistory ? [{ date: "2024-01-01", close: 10 }] : [],
            mini: { 7: [], 30: [], 180: [] },
            positions: [],
          }),
        } as Response);
